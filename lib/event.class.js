...
var Event = function Event(config) {
  ...
  _collides = function collides(event, _since, _till) {
    ...
    var collisions = [].concat(qs <= es && qt >= es ? ['left'] : []).concat(qs <= et && qt >= et ? ['right'] : []).concat(qs > es && qt < et ? ['inside'] : []).concat(qs < es && qt > et ? ['outside'] : []);
  ...
  var getSequels = function getSequels(_since, _till) {
    ...
    return sequels.reduce(function (events, sequel, index) {
      ...
      var realSequel = new Event(Object.assign({}, getPrivateObject(), sequel, {
        virtual: true,
    ...
    if (overlap.internal === 'trim') {
      return events.reduce(function (evts, event) {
        ...
        var trimmedSlaves = slaves.map(function (evt) {
          var slave = evt;
          var col = _collides(slave, master.since, master.till);
          while (col) {
            ...
            col = slave ? _collides(slave, master.since, master.till) : false;
          }
          return slave;
        }).filter(function (evt) {
